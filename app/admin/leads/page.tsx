...
export default function LeadManagement() {
    ...
    // Modal states
    const [viewModal, setViewModal] = useState<{ open: boolean; lead: Lead | null }>({
        open: false,
        lead: null,
    });
    const [deleteModal, setDeleteModal] = useState<{ open: boolean; leadId: string | null }>({
        open: false,
        leadId: null,
    });
    const [assignModal, setAssignModal] = useState<{ open: boolean; leadId: string | null }>({
        open: false,
        leadId: null,
    });
    ...
    return (
        <div className="space-y-6">
            ...
            {leadsData?.stats && (
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {Object.entries(leadsData.stats).map(([key, value]) => (
                        ...
                        >
                            <p className="text-sm font-sans text-text-muted uppercase">{key}</p>
                                ...
                                {leadsData?.data?.length === 0 ? (
                                    <tr>
                                        <td colSpan={8} className="px-6 py-12 text-center">
                                            <p className="text-text-muted font-sans">No leads found</p>
                                        </td>
                                    </tr>
                                ) : (
                        ...
                        <div className="border-t border-border px-6 py-4 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <p className="text-sm font-sans text-text-muted">
                                    ...
                                    <span className="font-semibold">
                                        {(leadsData.pagination.page - 1) * leadsData.pagination.limit + 1}
                                    </span>{" "}
                                    to{" "}
                                    <span className="font-semibold">
                                        {Math.min(
                                    ...
                                    of <span className="font-semibold">{leadsData.pagination.total}</span>{" "}
                                    leads
                                </p>
                                <select
                                    value={limit}
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: leadsData.pagination.totalPages }, (_, i) => i + 1)
                                        .filter(
                                            (p) =>
                                                p === 1 ||
                                        ...
                                        .map((p, i, arr) => (
                                            <>
                                                ...
                                                <button
                                                    key={p}
                                                    onClick={() => setPage(p)}
                                                    className={`px-3 py-2 rounded-lg text-sm font-sans font-semibold transition-colors ${p === leadsData.pagination.page
            ...
            {viewModal.open && viewModal.lead && (
                <Modal onClose={() => setViewModal({ open: false, lead: null })} title="Lead Details">
